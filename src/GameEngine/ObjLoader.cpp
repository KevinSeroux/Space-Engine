...
Mesh* ObjLoader::load(std::string& file)
{
	...
	while(getline(_objStream, file))
	{
		...
		if(_mtlStream)
		{
			...
			else if(file.substr(0,7)=="map_Kd ")
			{
			    /*std::string url;
			      sscanf(file.c_str(), "map_Kd %s", &url);
			      Engine::getInstance()->getMaterialManager().getMaterials()[_lastMtl].setTextureURL(url);*/
			}
		    }
		}
		else
